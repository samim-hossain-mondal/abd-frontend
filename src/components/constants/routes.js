export const WELCOME_ROUTE = '/';
export const LOGIN_ROUTE = "/login";
export const LOGIN_CALLBACK_ROUTE = "/login/callback";

export const HOME_ROUTE = '/home';
export const DAILY_ROUTE = '/daily';
export const PO_NOTE_ROUTE = '/po-notes';
export const OUR_TEAM_ROUTE = '/our-team';
export const CALENDAR_ROUTE = '/availability-calendar';
export const ANNOUNCEMENTS_ROUTE = '/announcements';
export const REFERENCE_MATERIALS_ROUTE = '/reference-material';
export const INFORMATION_RADIATORS_ROUTE = '/information-radiators';
export const MADE_TO_STICK_ROUTE = '/made-to-stick';

export const HOME_PAGE_NAME = 'Home';
export const DAILY_PAGE_NAME = 'Daily';

export const PO_NOTE_PAGE_NAME = 'PO Notes';
export const OUR_TEAM_PAGE_NAME = 'Our Team';
export const CALENDAR_PAGE_NAME = 'Availability Calendar';
export const ANNOUNCEMENTS_PAGE_NAME = 'Reminders';
export const REFERENCE_MATERIALS_PAGE_NAME = 'Reference Material';
export const INFORMATION_RADIATORS_PAGE_NAME = 'Information Radiators';
export const MADE_TO_STICK_PAGE_NAME = 'Made to Stick';

export const allPages = [
  // HOME_PAGE_NAME,
  DAILY_PAGE_NAME,
  OUR_TEAM_PAGE_NAME,
  MADE_TO_STICK_PAGE_NAME
]

export const UNAUTHORIZED_ROUTE = '/unauthorized';
export const NOT_FOUND_ROUTE = '*';
export const ERROR_ROUTE = '/error';


